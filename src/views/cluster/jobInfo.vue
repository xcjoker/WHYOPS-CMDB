<template>
	<frame>
		<div class="main-frame">
				...
				<el-card shadow="never" class="main-card">
					<div class="header-actions">
						...
						<div class="right-panel">
							<el-input v-model="searchKeyword" placeholder="搜索 Job..." class="search-input" clearable
								size="large" :prefix-icon="Search" />
							<el-button size="large" :icon="Refresh" circle @click="fetchData" class="refresh-btn" />

							<el-tooltip content="创建一次性任务" placement="top">
								<span>
					...
					<el-table :data="filteredList" v-loading="loading" style="width: 100%"
						...
						<el-table-column label="完成情况 (Succeeded)" width="180" align="center">
							<template #default="{ row }">
								<div class="replica-badge" :class="getBadgeClass(row.status)">
									{{ row.completions }}
								</div>
								<div class="sub-text">
									<span v-if="row.active > 0" class="text-primary">{{ row.active }} Running</span>
									<span v-if="row.failed > 0" class="text-danger ml-2">{{ row.failed }} Failed</span>
								</div>
							</template>
						</el-table-column>

						<el-table-column label="耗时" width="150" align="center">
							<template #default="{ row }">
			...
			<el-dialog v-model="createDialogVisible" title="创建 Job" width="60%" top="5vh" destroy-on-close>
				<el-alert title="Job 是一次性任务，Pod 成功结束后不会重启" type="info" show-icon :closable="false" class="mb-4" />
...
`
	...
	const fetchData = async () => {
		...
		loading.value = true
		try {
			const res = await workloadHttp.getJobs(queryParams.cluster_id, queryParams.namespace)
			dataList.value = res
	...
	// --- 操作逻辑 ---
	const handleCreate = () => {
		createYaml.value = defaultJobTemplate
	...
	const submitCreate = async () => {
		...
		try {
			await workloadHttp.createJob(queryParams.cluster_id, createYaml.value)
	...
	const handleViewYaml = async (row) => {
		...
		try {
			const res = await workloadHttp.getJobYaml(queryParams.cluster_id, row.namespace, row.name)
	...
	const handleUpdateYaml = async () => {
		...
		try {
			await ElMessageBox.confirm('Job 的部分字段不可变，通常建议删除重建。确定尝试更新吗？', '警告', {
				type: 'warning'
			})
			loadingInst = ElLoading.service({
				text: 'Updating...'
			})
			await workloadHttp.updateJobYaml(queryParams.cluster_id, currentRaw.value.namespace, currentRaw.value
	...
	const handleDelete = (row) => {
		ElMessageBox.confirm(`确认删除 Job: ${row.name}? 相关的 Pod 也会被清理。`, '删除确认', {
			type: 'warning',
			confirmButtonClass: 'el-button--danger'
		}).then(async () => {
			await workloadHttp.deleteJob(queryParams.cluster_id, row.namespace, row.name)
...
<style scoped>
	...
	.replica-badge.is-primary {
		background-color: #ecf5ff;
		color: #409EFF;
	}

	.replica-badge.is-danger {
